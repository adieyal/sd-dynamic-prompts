...
class TestPromptWriter:
    def _write_to_file(self, prompt_writer):
        with NamedTemporaryFile("w", encoding="utf-8", delete=True) as f:
            prompt_writer.write_prompts(f.name)

    def _checks_writes_empty_file(self, prompt_writer):
        with NamedTemporaryFile("w", encoding="utf-8", delete=True) as f:
            prompt_writer.write_prompts(f.name)
            output = open(f.name).read()
            return output == ""

    def test_default_disabled(self, prompt_writer):
        assert prompt_writer.enabled is False

    def test_reset(self, prompt_writer):
    ...
    def test_set_data(self, populated_prompt_writer):
    ...
    def test_doesnt_write_when_disabled(self, populated_prompt_writer):
        populated_prompt_writer.set_enabled(False)
        assert self._checks_writes_empty_file(populated_prompt_writer)

    def test_write_prompts(self, populated_prompt_writer):
    ...
    def test_only_write_once(self, populated_prompt_writer):
        populated_prompt_writer.set_enabled(True)

        self._write_to_file(populated_prompt_writer)
        assert self._checks_writes_empty_file(populated_prompt_writer)

    def test_writes_after_reset(self, populated_prompt_writer):
        populated_prompt_writer.set_enabled(True)
        self._write_to_file(populated_prompt_writer)
